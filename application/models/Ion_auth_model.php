...
class Ion_auth_model extends CI_Model
{
	...
	/**
 * Descrição da classe ou método, se aplicável.
 */
protected $db;
	...
	protected function _retrieve_selector_validator_couple($user_code)
{
    ...
    if ($user_code !== FALSE && is_string($user_code) && strpos($user_code, '.') !== FALSE) {
        $tokens = explode('.', $user_code);

        // Verifica se o código do usuário tem dois tokens
        if (count($tokens) === 2) {
            return (object) [
                'selector' => $tokens[0],
                'validator' => $tokens[1]
    ...
    // Retorna um objeto vazio em vez de FALSE
    return (object) [
        'selector' => '',
        'validator' => ''
    ];
